...
<div x-data="{ selected: @entangle('selected') }">
    <div class="flex justify-between items-center mb-4">
        <h2 class="text-xl font-bold">Items Herramientas Registradas</h2>
    </div>
    ...
    <div 
        ...
        x-on:herramienta-actualizado.window="items = $event.detail.items"
        class="space-y-2"
    >
        ...
        <div class="border-b bg-white dark:bg-gray-800 flex justify-between items-center mb-4">
            ...
            <x-filament::input
                ...
                placeholder="Ej: guante de latex"
                x-model="search"
                class="placeholder-gray-400 dark:placeholder-gray-500"
            />
        </div>

            <template x-for="herramienta in filtered" :key="herramienta.id">
                    ...
                    >
                        <div>
                            <button 
                                ...
                                @click="$wire.abrirModal(herramienta.id)"
                            >
                                <span x-text="herramienta.nombre"></span>
                            </button>
                            <span class="text-gray-500 max-w-xs truncate inline-block align-middle">
                                : <span x-text="herramienta.descripcion"></span>
                            </span>
                        </div>
                        <div class="px-4 py-2 text-center">
                        <x-filament::button 
                            color="primary" 
                            @click="$wire.abrirModal2(herramienta.id)"
                            size="sm">
                            Editar
                        </x-filament::button>
                        </div>
        ...
        @livewire('almacen.modal-herramientas') 
    </div>
        ...
        >
        <x-slot name="heading">
            <h3 class="text-lg font-semibold mb-4">Editar Herramienta</h3>
            {{ $this->item['nombre'] }}
        </x-slot>
            {{-- Contenido --}}
            ...
            <x-slot name="footer">
                    ...
                    @else
                        ...
                        <div class="mb-2 text-red-700 dark:text-red-400 font-semibold">
                            Si borras perderás todos los registros asociados a esta herramienta y no podrás recuperarlos.
                        </div>
